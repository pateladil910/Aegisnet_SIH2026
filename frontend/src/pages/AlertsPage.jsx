    ...
    <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6 space-y-6 font-sans">
      ...
      <div className="bg-white border border-slate-200/90 rounded-3xl p-6 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.05)] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-slate-50 border border-slate-200 px-4 py-2 rounded-2xl text-xs font-mono">
            <span className="text-slate-500">Pending Acknowledgment:</span>{' '}
            <b className={clsx(unackCount > 0 ? 'text-red-600' : 'text-emerald-600')}>
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-2 shadow-xs flex items-center gap-1.5 overflow-x-auto text-xs font-mono">
      ...
      <div className="space-y-4">
        {filteredAlerts.length === 0 ? (
          <div className="bg-white border border-slate-200/90 rounded-3xl p-12 text-center text-xs text-slate-500 space-y-1 shadow-xs">
            <span className="text-3xl block mb-2 text-emerald-600">✓</span>
            <div className="font-bold text-slate-900 text-base">No Active Incidents in Category</div>
          ...
          filteredAlerts.map((alert) => {
            ...
            return (
              <div
                key={alert.id}
                className={clsx(
                  'bg-white rounded-3xl p-6 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.05)] hover:shadow-md hover:border-blue-300 transition-all border space-y-4',
                  isEmergency
                    ? 'border-l-4 border-l-red-500 border-t-slate-200 border-r-slate-200 border-b-slate-200'
                    : isWarning
                    ? 'border-l-4 border-l-amber-500 border-t-slate-200 border-r-slate-200 border-b-slate-200'
                    : 'border-slate-200/90'
                )}
              >
                {/* Header Row */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl p-2 rounded-2xl bg-slate-100">{catInfo?.icon || '🚨'}</span>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-xs text-slate-900">{alert.id}</span>
                        <span
                          className={clsx(
                      ...
                      <h2 className="text-base font-bold text-slate-900 mt-1">{alert.title}</h2>
                    </div>
                  </div>

                  <div className="text-left sm:text-right text-xs font-mono text-slate-500">
                    <div>Detected: <b className="text-slate-700">{alert.timestamp}</b></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                  <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl space-y-1">
                    <span className="text-[10px] font-mono uppercase text-slate-500 font-bold">Affected Landmark</span>
                    <div className="font-semibold text-slate-900 text-sm">{alert.location}</div>
                    <div className="text-[11px] text-blue-600 font-mono">{alert.landmark_tag}</div>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl space-y-1 md:col-span-2">
                    <span className="text-[10px] font-mono uppercase text-slate-500 font-bold">Root Cause Analysis</span>
                    <p className="text-slate-600 leading-relaxed">{alert.root_cause}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
                  <div className="text-xs font-mono text-slate-500 flex items-center gap-2">
                    ...
                    <b className="text-slate-900 bg-slate-100 px-2.5 py-1 rounded-full text-[11px]">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {!alert.acknowledged ? (
                      ...
                      <span className="text-xs text-emerald-700 font-mono font-bold bg-emerald-50 border border-emerald-200 px-3.5 py-1.5 rounded-full">
                        ✓ Acknowledged
                      </span>
                    )}
                    ...
                    <button
                      ...
                      className="bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 hover:text-blue-600 text-xs font-mono font-bold px-4 py-2 rounded-full transition-all shadow-xs"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 backdrop-blur-md p-4 animate-fade-in font-sans">
          <div className="bg-white border border-slate-200/90 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-bold text-base text-slate-900">Incident Acknowledgment Audit</h3>
              <button onClick={() => setSelectedAlertForAck(null)} className="w-8 h-8 rounded-full flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors">
                ✕
              </button>
            </div>
            ...
            <div className="space-y-2 text-xs">
              <label className="text-slate-700 font-mono block font-bold">Duty Officer Credential</label>
              <input
                type="text"
                value={officerName}
                onChange={(e) => setOfficerName(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 font-mono text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 focus:outline-none"
              />
            </div>
            ...
            <div className="pt-2 flex gap-2">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white font-bold py-2.5 rounded-full text-xs font-mono shadow-md shadow-blue-500/25 transition-all"
              >
                Confirm Acknowledgment
              </button>
              <button
                onClick={() => setSelectedAlertForAck(null)}
                className="px-5 bg-slate-100 text-slate-700 rounded-full text-xs font-mono border border-slate-200 hover:bg-slate-200 transition-colors"
              >
                Cancel
              </button>
            </div>
